...
title('Occupancy Grid and Paths - Q8');
setActions;

for m=1:3
     replan_index = 1;
    [curr_loc, goal_loc] = assignStartEndLocs_Q5(m);
...
%     [curr_ind, goal_ind] = getStartEndIndicesQ3_func(grids, curr_loc, goal_loc);
    ...
    while sum(curr_ind == goal_ind) ~= 2 %has not reached goal yet. 
        ...
        explored(rem_ind, :) = [];
        if replan_index ~= 1 && ~isempty(rem_ind) %drive from prev loc to center of the first grid of the plan. 
...
%         disp(waypoints);
        ...
        %updating the current grid, if not continue driving. 
...
%         wayposes = [waypoints, ang_list];
        ...
        for i=1:size(wayposes, 1)-1
            ...
            %forward simulate controls from controller.
            for j=1:size(control_list,1)
                ...
                if ismember(traj_index, explored, 'rows') == 0 %check if the current trajectory has deviated from the original plan, if so, replan
                    isReplan = true;
