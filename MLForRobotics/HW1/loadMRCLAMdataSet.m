...
[subject_num, barcode_num] = textread('ds1_Barcodes.dat', '%u %u','commentstyle','shell');
Barcodes = [subject_num, barcode_num];
clear subject_num barcode_num;

disp('Reading landmark groundtruth')
[subject_num x y x_sd y_sd] = textread('ds1_Landmark_Groundtruth.dat', '%d %f %f %f %f','commentstyle','shell');
...
for i=1:n_robots
    ...
    [time x y theta] = textread('ds1_Groundtruth.dat', '%f %f %f %f','commentstyle','shell');
    eval(['Robot' num2str(i) '_Groundtruth = [time x y theta];']); 
    clear time x y theta;

    disp(['Reading robot ' num2str(i) ' odometry'])
    [time, v, w] = textread('ds1_Odometry.dat', '%f %f %f','commentstyle','shell');
    eval(['Robot' num2str(i) '_Odometry = [time v w];']);
    clear time v w;
    
    disp(['Reading robot ' num2str(i) ' measurements'])
    [time, barcode_num, r b] = textread('ds1_Measurement.dat', '%f %f %f %f','commentstyle','shell');
